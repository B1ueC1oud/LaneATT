...
def parse_args():
    ...
    parser.add_argument("mode", choices=["train", "test","webcam"], help="Train or test?")
    parser.add_argument("--exp_name", help="Experiment name", required=True)
    parser.add_argument("--cfg", help="Config file")
    parser.add_argument("--web",type=str,default="None", help="Config file")
...
def main():
    ...
    if args.web == 'webcam':
        runner = Runner(cfg, exp, device,args.test_dataset,args.test_first_dir,args.test_second_dir,args.exp_name,hyper,hyper_param, args.video_name,args.data_dir, webcam=True, view=args.view, resume=args.resume, deterministic=args.deterministic)
    else:
    ...
    if args.web=='webcam':
        runner.webcam(epoch=args.epoch or exp.get_last_checkpoint_epoch(), save_predictions=args.save_predictions)
    else:
