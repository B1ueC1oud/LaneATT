...
1. When testing unlabeled datasets other than Private datasets, there are two methods.
    - After setting the hyperparameter **dataset** to **kodasv1**, put the absolute address to the folder containing **.jpg** in hyperparameter **data_dir**(ex: "/data/lanedata/private").
    - Or change **"Private_LaneATT/lib/datasets/nolabel_dataset.py"** file. (you need to change function **load_annotations**)

2. If you set the hyperparameter as below, the address where the ouput image or video will be saved is **"/data/lane_detection/result_Private/laneatt_r18_culane/conf_threshold_0.4_nms_thres_45.0_max_lane_2/"**.
